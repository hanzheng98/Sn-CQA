...
parser.add_argument('--J', type=list, default=[1.0, 0.5])
...
def train(model, trial_state, optimizer, observable, multi_factor: int, device=None, debug=False):
    ...
    optimizer.zero_grad()
    x = model(trial_state)
    observable = observable + (torch.ones(observable.shape[0], device=device)) * multi_factor
    observable = torch.view_as_complex(torch.stack([observable, torch.zeros_like(observable, device=device)], dim=-1))
...
def main():
    ...
    observable = model.Heisenberg.to(device)
    
    EDvalues, EDvectors = torch.linalg.eig(model.Heisenberg.to(device))
    EDvector = EDvectors[torch.argmin(torch.real(EDvalues))]
    EDvalue = EDvalues[torch.argmin(torch.real(EDvalues))]
    
    init_state = get_basis(model.dim, args.sample_size).to(device)
    ...
    for i in range(1, args.numiter + 1):
        ...
        loss = train(model, init_state, optimizer, observable, len(lattice[0]) + len(lattice[1]), device=device, debug=False)
